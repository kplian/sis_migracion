...
DECLARE
		...
		BEGIN
		  ...
		  ELSE 
		      v_consulta =  ' SELECT migracion.f_trans_tpr_concepto_ingas_tconcepto_ingas (
		              '''||TG_OP::varchar||''','||OLD.id_concepto_ingas||',NULL,NULL,'||COALESCE(OLD.id_partida::varchar,'NULL')||',NULL,'||COALESCE(''''||OLD.desc_ingas::varchar||'''','NULL')||',NULL,NULL,NULL,NULL,NULL,NULL) as res';
