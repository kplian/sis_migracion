		...
		BEGIN
		   IF(TG_OP = 'INSERT' or  TG_OP ='UPDATE' ) THEN
			 ...
			 v_consulta =  'SELECT migracion.f_trans_tts_cuenta_bancaria_tts_cuenta_bancaria (
                  '''||TG_OP::varchar||''','||COALESCE(NEW.id_cuenta_bancaria::varchar,'NULL')||','||COALESCE(NEW.id_auxiliar::varchar,'NULL')||',
                  '||COALESCE(NEW.id_cuenta::varchar,'NULL')||',
                  '||COALESCE(NEW.id_institucion::varchar,'NULL')||',
                  '||COALESCE(NEW.id_parametro::varchar,'NULL')||',
                  '||COALESCE(NEW.estado_cuenta::varchar,'NULL')||',
                  '||COALESCE(NEW.nro_cheque::varchar,'NULL')||',
                  '||COALESCE(''''||NEW.nro_cuenta_banco::varchar||'''','NULL')||',
                  '||COALESCE(v_id_gestion::varchar,'NULL')||',
                  '||COALESCE(''''||NEW.central::varchar||'''','NULL')||') as res';				
